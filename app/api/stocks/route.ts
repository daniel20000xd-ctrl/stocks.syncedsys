...
import { fetchRankedNews } from '@/lib/news'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // ── News (ranked for ticker relevance — shared with /api/news live poll) ────
    const news: StockResult['news'] = await fetchRankedNews(ticker, companyName)
    ...
    const result: StockResult = {
      ...
      companyName, description, sector, industry, employees, website, country,
      nextEarningsDate,
      avanza,
      ohlcv, sma50, sma200,
      indicators: {
